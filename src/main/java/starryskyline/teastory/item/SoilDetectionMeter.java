...
public class SoilDetectionMeter extends TSItem
{
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
		if(worldIn.isRemote)
		{
            ...
            int height = pos.getY();
            playerIn.sendMessage(new TextComponentTranslation("teastory.soil_detection_meter.message.total", temp, rainfall, ("\u00a78" + String.valueOf(height)), ("\u00a7b" + biome.getBiomeName())));
            if ((temperature >= 0.5F))
            {
            	if(humidity <0.5)
            	{
            	    playerIn.sendMessage(new TextComponentTranslation("teastory.soil_detection_meter.message.drying"));
            	}
            	else playerIn.sendMessage(new TextComponentTranslation("teastory.soil_detection_meter.message.fermentation"));
            }
            ...
            playerIn.sendMessage(new TextComponentTranslation("teastory.soil_detection_meter.message.teaplant", f, h));
